...
def get_results(args):
    ...
    for fold, (f_times, f_days) in enumerate(folds):
        ...
        eval_groups = [("test",test_labels, test_classes, test_mat),
                       ("validation", val_labels, val_classes, val_mat)]
        
        for (t_rel, t_labels, t_classes, t_mat,) in eval_groups:
                ...
                with seamr.BlockWrapper("%s: Predicting %s data" % (fold_name, t_rel,)):
                    ...
                    for i,cls in enumerate(t_classes):
                        if cls != "other" and classifiers.get(cls, None) is not None:
                            try:
                                t_probs[:,i] = classifiers[cls].predict_proba(t_mat)[:,1]
                            except IndexError:
                                pass
...
with open( os.path.join(conf['results'], '%s_seb.csv' % run_name), "wt" ) as fOut:
    writer = None
    with Pool() as pool:
        for resblock in pool.imap_unordered(get_results, run_args):
            if writer is None:
                fields = set( resblock[0].keys() )
                writer = csv.DictWriter( fOut, sorted(fields - {"f1Curve"}), extrasaction="ignore" )
                writer.writeheader()
