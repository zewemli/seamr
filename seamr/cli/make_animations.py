...
def create_all_animations(pArgs):
    ...
    for tSensors, ts in core.gen_active_sensors(lambda: store.get_events(dset, days=sorted(days)), times):
        ...
        ids = set([ l.line for l in labels ])

        for k in list(openVideos.keys()):
            if k not in ids:
                openVideos.pop(k).close()

                print("Done with %s" % lineName[k])
        
        for l in labels:
            if l.activity != "other" and l.line not in openVideos:
                lPath = os.path.join("videos", dset, l.resident, l.activity)
                if not os.path.exists( lPath ):
                    os.makedirs(lPath)

                dt = datetime.fromtimestamp(l.start)
                sec = l.finish - l.start

                vname = os.path.join(lPath, "%s-%s-L%06d-sec-%s.mp4" % ( dset, dt, l.line, int(sec) ))
                lineName[l.line] = vname

                openVideos[l.line] = skvideo.io.FFmpegWriter( vname,
                                                inputdict={'-r': "2/1"},
                                                outputdict={'-r': "2/1"})

                print("Opening %s" % lineName[l.line])
    
        if any([ l.activity != "other" for l in labels ]):
            if img is None or tSensors != prevSensors:
                img = res.getImage(tSensors, scale=scale)

            for vid in openVideos.values():
                vid.writeFrame(img)
...
parser.add_argument("--frames", default=600, type=int, help="Number of animations to produce")
...
pargs = [(store,
          ...
          max_steps,
          gridSize,
          args.num, args.frames, args.scale)
