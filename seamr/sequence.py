...
class SeqEstimator:
    ...
    def __init__(self, n_components = 64, step_low = 8, step_high = 32):
    ...
    def fit(self, sequence):
        ...
        lens = []
        e = 0
        l = random.randint(self.step_low, self.step_high)
            
        while (e + l) < len(sequence):
            e += l
            lens.append(l)
            l = random.randint(self.step_low, self.step_high)
        
        eSum = sum(lens)
        if eSum < len(sequence):
            lens.append( len(sequence) - eSum )

        model.fit(sequence, lens)

    def probability(self, seq):
         self.model.score( seq ) / len( seq )

    def group_prob(self, seq, groups, xform = lambda t: t):
        return [ (gg, self.probability([ s for s,g in items ]))
                    for gg,items in groupby(zip(seq,groups), key=lambda T: xform(T[1])) ]
...
def modelObjectSeq(args):
    ...
    sensorIDs = [ s.id for s in store.get_sensors(dset) ]
